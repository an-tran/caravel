...
Revision ID: bcf30fc23f78
Revises: 3b626e2a6783
Create Date: 2016-09-29 14:58:28.079061

"""

# revision identifiers, used by Alembic.
revision = 'bcf30fc23f78'
...
def upgrade():
    ...
    op.alter_column('columns', 'created_on',
               existing_type=sa.DATETIME(),
               nullable=True)
    op.create_foreign_key(None, 'columns', 'datasources', ['datasource_name'], ['datasource_name'])
...
def downgrade():
    ...
    op.alter_column('sql_metrics', 'changed_on',
               existing_type=sa.DATETIME(),
               nullable=False)
    op.add_column('slices', sa.Column('table_id', sa.INTEGER(), nullable=True))
    op.add_column('slices', sa.Column('druid_datasource_id', sa.INTEGER(), nullable=True))
    op.create_foreign_key(None, 'slices', 'datasources', ['druid_datasource_id'], ['id'])
    op.create_foreign_key(None, 'slices', 'tables', ['table_id'], ['id'])
    op.alter_column('slices', 'created_on',
               existing_type=sa.DATETIME(),
               nullable=False)
    op.alter_column('slices', 'changed_on',
